#!/usr/bin/env python3
"""
Swyftx Webhook Handler
Handles real-time notifications from Swyftx
"""
...
class SwyftxWebhookHandler:
    ...
    def setup_logging(self):
        ...
        self.logger = logging.getLogger(f"swyftx_webhook")
    
    def verify_signature(self, payload: str, signature: str) -> bool:
        """Verify webhook signature"""
        try:
            secret = os.getenv('SWYFTX_WEBHOOK_SECRET', '')
...
handler = SwyftxWebhookHandler()

@app.route('/webhook/swyftx/order', methods=['POST'])
def order_webhook():
...
@app.route('/webhook/swyftx/balance', methods=['POST'])
def balance_webhook():
