...
# OpenRouter API Keys - Load from config.json or use defaults
if config and 'openrouter_api_keys' in config:
    OPENROUTER_API_KEYS = config['openrouter_api_keys']
    print(f"✅ Loaded {len(OPENROUTER_API_KEYS)} API keys from config.json")
else:
    # Fallback to hardcoded keys if config.json not found
    OPENROUTER_API_KEYS = [
    ...
    print("⚠️  Using default API keys (config.json not found)")
